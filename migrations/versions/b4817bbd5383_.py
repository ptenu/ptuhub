...
def upgrade():
    ...
    sa.ForeignKeyConstraint(['_reported_by'], ['contacts.id'], onupdate='CASCADE', ondelete='SET NULL'),
    sa.PrimaryKeyConstraint('date', 'reference')
    )
    op.drop_constraint('contacts_lives_at_fkey', 'contacts', type_='foreignkey')
    op.create_foreign_key(None, 'contacts', 'contact_addresses', ['lives_at'], ['id'], onupdate='CASCADE', ondelete='SET NULL')
    op.alter_column('requests', 'session_id',
...
def downgrade():
    ...
    op.create_foreign_key('contacts_lives_at_fkey', 'contacts', 'contact_addresses', ['lives_at'], ['id'])
